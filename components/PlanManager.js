    ...
    return (
        <div style={{
            ...
            <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
                <h2 style={{ margin: 0 }}>My Workout Plans</h2>
                <button onClick={onClose} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', fontSize: '1rem' }}>Close</button>
            </header>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '12px' }}>
                            <div>
                                <h3 style={{ margin: 0, fontSize: '1.2rem', color: plan.is_active ? 'var(--primary)' : 'white' }}>
                                    {plan.name} {plan.is_active && <span style={{ fontSize: '0.8rem', background: 'var(--primary)', color: 'black', padding: '2px 8px', borderRadius: '100px', marginLeft: '8px' }}>Active</span>}
                                </h3>
                        ...
                        <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', paddingBottom: '8px', marginBottom: '16px' }}>
                            {(plan.days || []).map(day => (
                                <div key={day.id} style={{
                                    ...
                                    <div style={{ fontWeight: 'bold', color: 'var(--text-muted)', fontSize: '0.7rem' }}>Day {day.day_order}</div>
                                    <div>{day.label}</div>
                                </div>
                            ))}
                        </div>
