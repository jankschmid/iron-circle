    ...
    return (
        <header className="flex items-center justify-between px-6 py-5 border-b border-white/5 bg-black/40 backdrop-blur-2xl sticky top-0 z-50 transition-all">
            ...
            <nav className="flex items-center gap-6 sm:gap-8 text-sm font-medium relative z-10">
                <a href="#b2c" className="text-zinc-400 hover:text-white transition-colors hidden sm:block">Athleten</a>
                <a href="#b2b" className="text-zinc-400 hover:text-white transition-colors hidden sm:block">Studio-Partner</a>
                
                {user ? (
                    <>
                        <Link href="/dashboard" className="text-zinc-400 hover:text-white transition-colors">Dashboard</Link>
                        <Link href="/dashboard" className="relative group overflow-hidden px-5 py-2.5 bg-brand/10 border border-brand/50 hover:bg-brand/20 text-brand font-semibold rounded-full transition-all duration-300 shadow-md">
