    ...
    return (
        <footer className="px-8 flex flex-col items-center py-8 bg-black border-t border-white/10 text-center text-zinc-500 text-sm">
            ...
            <div className="flex flex-wrap justify-center gap-8 font-medium mb-6">
                ...
                <Link href="/feedback" className="hover:text-brand transition-colors">Leave a Review</Link>
                <Link href="/contact" className="hover:text-white transition-colors">Contact Support</Link>
                <Link href="/login" className="hover:text-brand transition-colors">Gym Admin Login</Link>
            </div>
            <span className="font-semibold tracking-[0.2em] text-[10px] uppercase">© {new Date().getFullYear()} IRONCIRCLE. BUILT FOR ATHLETES.</span>
        </footer>
    );
}
