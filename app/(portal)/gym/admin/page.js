...
function GymAdminPage() {
    ...
    const [isTrainerOnly, setIsTrainerOnly] = useState(false);
    ...
    const fetchGymDetails = async () => {
        try {
            ...
            const impersonationGymId = typeof window !== 'undefined' ? sessionStorage.getItem('support_impersonation_gym_id') : null;
            if (impersonationGymId === gymId && user.is_super_admin && gymData.support_access_active && new Date(gymData.support_expires_at) > new Date()) {
                hasAccess = true;
            }
            ...
            if (!hasAccess) {
                ...
                if (roleData && (roleData.role === 'admin' || roleData.role === 'owner' || roleData.role === 'trainer')) {
                    hasAccess = true;
                    if (roleData.role === 'trainer' && !user.is_super_admin && gymData.created_by !== user.id) {
                        setIsTrainerOnly(true);
                    }
                }
            }
        ...
        <div className="dashboard-container" style={{ minHeight: '100vh', background: '#000', color: '#fff', fontFamily: 'Inter, sans-serif', display: 'flex' }}>
            ...
            <aside className="dashboard-sidebar" style={{ width: '250px', borderRight: '1px solid #222', padding: '24px', display: 'flex', flexDirection: 'column' }}>
                <div style={{ marginBottom: '40px' }}>
                    ...
                    {typeof window !== 'undefined' && sessionStorage.getItem('support_impersonation_gym_id') === gym.id && (
                        <div style={{ marginTop: '16px', background: '#4a0000', border: '1px solid #ff0000', color: '#ffaaaa', padding: '8px', borderRadius: '8px', fontSize: '0.8rem' }}>
                            <strong>🛠️ Support Mode Active</strong>
                            <div style={{ marginTop: '4px', cursor: 'pointer', textDecoration: 'underline' }} onClick={() => {
                                sessionStorage.removeItem('support_impersonation_gym_id');
            ...
            <main className="dashboard-content" style={{ flex: 1, padding: '40px', overflowY: 'auto' }}>
                ...
                {activeTab === 'settings' && (
                    <SettingsTab gym={gym} supabase={supabase} fetchGymDetails={fetchGymDetails} />
                )}
            </main>
