...
export function getSmartSuggestion(setsArray, assignment = null) {
    ...
    if (bestSet.failed || (bestSet.rpe && parseFloat(bestSet.rpe) > 9.5)) {
        return {
            ...
            type: 'maintain',
            reason: `Smart Recovery 🛡️ — ease off slightly to come back stronger`,
            isPush: false
        };
    }
...
export function calculateNextSet(lastWeight, lastReps, focusMode, progressionStep = 1.25) {
    ...
    // Rule A: Hit the top of the range → increase weight next session
    if (lastReps >= maxTargetReps) {
        return {
            weight: lastWeight + progressionStep,
            reps: minTargetReps,
            reason: `Progressive Overload! 🏆 +${progressionStep}kg — start fresh at ${minTargetReps} reps`,
            type: 'push',
            isPush: true
        };
    }

    // Rule B: Inside the range → keep same weight, push for the top
    if (lastReps >= minTargetReps && lastReps < maxTargetReps) {
        return {
            weight: lastWeight,
            reps: maxTargetReps,   // Target the TOP of the range, not just +1
            reason: `Building Volume 💪 — hit ${maxTargetReps} reps to unlock +${progressionStep}kg`,
            type: 'build',
            isPush: true
        };
    }

    // Rule C: Below the minimum → consolidate at the bottom of range
    if (lastReps < minTargetReps) {
        return {
            weight: lastWeight,
            reps: minTargetReps,
            reason: `Consolidating 🔥 — target ${minTargetReps} reps with solid form`,
            type: 'maintain',
            isPush: false
        };
    }
