...
export function calculateWorkoutImpact(session, exercisesRegistry = null, nowTime = Date.now()) {
    ...
    for (const log of logs) {
        ...
        if (!exerciseName && log.exerciseId && exercisesRegistry) {
            definedEx = exercisesRegistry.find(e => e.id === log.exerciseId);
            if (definedEx) exerciseName = definedEx.name;
        }

        if (!exerciseName) continue;

        const muscleEngagementMap = getMusclesForExercise(definedEx || exerciseName);
        if (Object.keys(muscleEngagementMap).length === 0) continue;
        ...
        for (const set of log.sets) {
            ...
            for (const [muscleId, intensity] of Object.entries(muscleEngagementMap)) {
                muscleVolume[muscleId] = (muscleVolume[muscleId] || 0) + volume * intensity;
            }
        }
    }
...
export function calculateTemplateImpact(template, exercisesRegistry = null) {
    ...
    template.exercises.forEach(ex => {
        ...
        if (!exerciseName && ex.id && exercisesRegistry) {
            definedEx = exercisesRegistry.find(e => e.id === ex.id);
            if (definedEx) exerciseName = definedEx.name;
        }

        if (exerciseName) {
            const muscleEngagementMap = getMusclesForExercise(definedEx || exerciseName);
            for (const [m, intensity] of Object.entries(muscleEngagementMap)) {
                heatMap[m] = Math.max(heatMap[m] || 0, intensity);
            }
        }
    });
