...
export const LOGICAL_MUSCLES = [
    'chest', 'front_delts', 'rear_delts', 'triceps', 'biceps', 'forearms',
    'lats', 'traps', 'mid_back', 'lower_back', 'abs', 'obliques',
...
export const EXERCISE_MUSCLE_MAP = {
    ...
    'dumbbell bench press': { primary: ['chest', 'triceps'],    secondary: ['delts_front'] },
    ...
    'push ups':             { primary: ['chest', 'triceps'],    secondary: ['delts_front'] },
    ...
    'arnold press':         { primary: ['delts_front'],         secondary: ['triceps'] },
    'lateral raises':       { primary: ['delts_rear'],          secondary: ['traps'] },
    'lateral raise':        { primary: ['delts_rear'],          secondary: ['traps'] },
    'front raises':         { primary: ['delts_front'],         secondary: [] },
    'front raise':          { primary: ['delts_front'],         secondary: [] },
    'rear delt fly':        { primary: ['delts_rear'],          secondary: ['traps'] },
    'upright row':          { primary: ['traps', 'delts_front'], secondary: ['biceps'] },
    ...
    // PULL
    'pull ups':             { primary: ['lats', 'biceps'],      secondary: ['traps', 'mid_back'] },
    'pull up':              { primary: ['lats', 'biceps'],      secondary: ['traps', 'mid_back'] },
    'chin ups':             { primary: ['lats', 'biceps'],      secondary: ['traps'] },
    ...
    'seated cable row':     { primary: ['mid_back', 'lats'],    secondary: ['biceps', 'traps'] },
    'barbell row':          { primary: ['lats', 'mid_back'],    secondary: ['biceps', 'lower_back'] },
    'dumbbell row':         { primary: ['lats', 'mid_back'],    secondary: ['biceps'] },
    't-bar row':            { primary: ['mid_back', 'lats'],    secondary: ['biceps', 'lower_back'] },
    'face pulls':           { primary: ['delts_rear', 'traps'], secondary: [] },
    'face pull':            { primary: ['delts_rear', 'traps'], secondary: [] },
    'shrugs':               { primary: ['traps'],               secondary: [] },
    'barbell shrugs':       { primary: ['traps'],               secondary: [] },
    'celso shrugs':         { primary: ['traps'],               secondary: [] },
    ...
    'muscle up':            { primary: ['lats', 'chest', 'triceps', 'biceps'], secondary: ['core'] },
    ...
    'lunges':               { primary: ['quads', 'glutes'],     secondary: ['hamstrings', 'adductors'] },
    ...
    'calf raises':          { primary: ['calves'],              secondary: ['shins'] },
    ...
    'russian twists':       { primary: ['obliques'],            secondary: [] },
    'russian twist':        { primary: ['obliques'],            secondary: [] },
    'hanging leg raises':   { primary: ['abs', 'adductors'],    secondary: [] },
    'leg raise':            { primary: ['abs', 'adductors'],    secondary: [] },
    'ab wheel':             { primary: ['abs'],                 secondary: ['lower_back'] },
    'ab wheel rollout':     { primary: ['abs'],                 secondary: ['lower_back'] },
};
...
/**
 * Get muscle IDs and their intensities for an exercise name or exercise object.
 * @param {string|Object} exerciseInput 
 * @returns {Object} Object mapping SVG path IDs to intensities (0.0 - 1.0)
 */
export function getMusclesForExercise(exerciseInput) {
    if (!exerciseInput) return {};

    // 1. New Engine: Use the JSONB muscle_engagement object
    if (typeof exerciseInput === 'object' && exerciseInput.muscle_engagement && Object.keys(exerciseInput.muscle_engagement).length > 0) {
        const result = {};
        for (const [logicalGroup, intensity] of Object.entries(exerciseInput.muscle_engagement)) {
            const svgIds = getSvgIdsForMuscle(logicalGroup);
            svgIds.forEach(id => {
                result[id] = Math.max(result[id] || 0, intensity);
            });
        }
        return result;
    }

    // 2. Legacy Engine: Fallback to primary/secondary arrays
    let primary = [];
    let secondary = [];

    if (typeof exerciseInput === 'object') {
        const p = exerciseInput.primary_muscles || [];
        const s = exerciseInput.secondary_muscles || [];
    ...
    if (primary.length === 0 && secondary.length === 0) {
        const name = typeof exerciseInput === 'string' ? exerciseInput.toLowerCase().trim() : '';
        if (name) {
    ...
    // Convert legacy arrays to the new intensity map
    const result = {};
    const pUnique = [...new Set(primary)];
    const sUnique = [...new Set(secondary)];

    pUnique.forEach(logicalGroup => {
        const svgIds = getSvgIdsForMuscle(logicalGroup);
        svgIds.forEach(id => result[id] = 1.0);
    });

    sUnique.forEach(logicalGroup => {
        const svgIds = getSvgIdsForMuscle(logicalGroup);
        svgIds.forEach(id => result[id] = Math.max(result[id] || 0, 0.4));
    });
...
export const WORKOUT_PLANS = {
    ...
    'Pull Day': {
        muscles: { 
            lats: 0.9, mid_back: 0.8, 
            biceps: 0.7, traps: 0.6, forearms: 0.4 
        },
        view: 'rear'
    },
    'Leg Day': {
        muscles: { 
            ...
            adductors: 0.6, calves: 0.5
        },
        view: 'front'
    },
    'Full Body': {
        muscles: { 
            chest: 0.6, lats: 0.6, quads: 0.7,
    ...
    'Upper Body': {
        muscles: {
            ...
            biceps: 0.7, triceps: 0.7, lats: 0.6,
