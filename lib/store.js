...
export function StoreProvider({ children }) {
    ...
    const fetchProfile = async (session) => {
        ...
        try {
            ...
            if (profile) {
                const totalXP = profile.lifetime_xp || 0;
                const calc = getLevelProgress(totalXP);
    ...
    const finishWorkout = async ({ visibility = 'public' } = {}) => {
        ...
        const { total: earnedXP, breakdown } = calculateSessionXP({
            ...
            // Heuristic for Cardio: >10 mins duration, 0 volume = Cardio session
        ...
        if (earnedXP > XP_CAP) {
            ...
            finalXP = XP_CAP;
            // Optionally flag session here
        }

        const lifetimeXP = user.lifetime_xp || user.xp || 0;
        const newLifetimeXP = lifetimeXP + finalXP;

        // NEW: Calculate Level & Progress using Scaling Formula
        const progressData = getLevelProgress(newLifetimeXP);
        const newLevel = progressData.currentLevel;
        const newCurrentXP = progressData.progress;
        const didLevelUp = newLevel > (user.level || 1);

        console.log(`Gamification: Earned ${finalXP} XP. Level: ${newLevel}`);
