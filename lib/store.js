...
export function StoreProvider({ children }) {
    ...
    const fetchProfile = async (session) => {
        // Fix: Do not fetch profile on public Monitor pages to avoid 400/Abort errors
        if (pathname?.startsWith('/gym/display') || pathname?.startsWith('/community')) return null;
        ...
        try {
            ...
            if (gymIds.length > 0) {
                try {
                    // 1s timeout for coordinates - non-critical
                    ...
                    const { data: coordsData, error: coordsError } = await Promise.race([coordsPromise, timeoutPromise]);
                    ...
                    console.warn('RPC get_gym_coordinates failed or timed out:', err);
                }
            }

            // Fetch Community Roles & Types
            const { data: rolesData } = await supabase
                .from('community_members')
                .select('community_id, role, communities(gym_id, gym_type)')
                .eq('user_id', session.user.id);

            const roleMap = {}; // gym_id -> { role, type }
            rolesData?.forEach(r => {
                if (r.communities?.gym_id) {
                    roleMap[r.communities.gym_id] = {
                        role: r.role,
                        type: r.communities.gym_type
                    };
                }
            });
            ...
            const gyms = userGymsData?.map(ug => ({
                ...
                radius: ug.radius || 200, // Default 200m
                gymType: roleMap[ug.gym_id]?.type || 'community', // Map Gym Type from roles
                role: roleMap[ug.gym_id]?.role || 'member' // 'member', 'trainer', 'admin'
            })) || [];
            ...
            if (profile) {
                const sessionUser = {
                    ...
                    is_super_admin: profile.is_super_admin || false, // CRITICAL: Verify Master Admin logic depends on this
                    // Note: prs removed or kept empty as placeholder
                    prs: {}
                };

                // Persist to Native Storage
                PreferenceUtils.set(KEYS.USER_SESSION, sessionUser).catch(err => console.error("Pref Save Error:", err));

                return sessionUser;
            } else if (error) {
                // Check if it's a "No rows" error (which is fine for new users) or a real error
                // code 'PGRST116' or similar is "JSON object requested, multiple (or no) rows returned"
                // But maybeSingle returns null data and null error for empty result? No, probably null error.
                // Wait, maybeSingle returns null data without error if 0 rows. 
                // If error exists here, it's a REAL DB error.
                console.error("Critical Profile Fetch Error:", error);
                // Fallback to offline user, DO NOT redirect to setup which assumes 'New User'
                return {
                    id: session.user.id,
                    email: session.user.email,
                    name: 'User (Offline)',
                ...
                // Only if NO profile and NO error -> New User
    ...
    // Timeout wrapper for fetchProfile
    const fetchProfileWithTimeout = async (session) => {
    ...
    useEffect(() => {
            ...
            if (event === 'SIGNED_IN' || event === 'TOKEN_REFRESHED' || event === 'INITIAL_SESSION') {
                if (session?.user) {
                    if (mounted) {
                        ...
                        // Only redirect to home if we are currently on auth pages
        ...
        const initAuth = async () => {
            ...
            // Get initial session manually to prevent flash, but let onAuthStateChange handle most updates
            // Note: onAuthStateChange fires INITIAL_SESSION in some versions, but getSession is safer for SSR/hydration sync
            ...
            if (session?.user) {
                // await handleAuthChange('INITIAL_SESSION', session); // Duplicate call?
                // Let's just fetch directly to avoid event double-fire if onAuthStateChange is fast
                const appUser = await fetchProfileWithTimeout(session);
    ...
    const fetchTemplates = async () => {
        if (!user || pathname.startsWith('/gym/display') || pathname.startsWith('/community')) return;
        const { data, error } = await supabase
