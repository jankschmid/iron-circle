...
export function StoreProvider({ children }) {
    ...
    const deleteSession = async (sessionId) => {
        try {
            ...
            // Update local state
            // history holds the sessions in tracker/page.js (mapped as 'history')
            // Actually, wait. 'history' in store usually refers to completed workouts?
            // Let's check if 'history' contains workout_sessions.
            // In tracker/page.js: "const { history } = useStore();"
            // And it renders: {history.map(session => ...
            // So yes, 'history' likely holds these sessions for the tracker context.
            // BUT usually 'history' is past workouts. Tracker sessions are different?
            // Tracker page seems to use 'history' for "Recent Sessions".
            // Let's assume 'history' is the correct state variable.
    ...
    const deleteWorkoutHistory = async (workoutId) => {
        ...
        try {
            console.log("Attempting RPC delete for:", workoutId);
            const { error } = await supabase.rpc('delete_workout', { target_workout_id: workoutId });
            ...
            // Re-fetch to restore state if needed (optional)
        }
    };
    ...
    const deleteGlobalGym = async (gymId) => {
        try {
            console.log("Deleting global gym:", gymId);
            // This will fail if foreign keys exist (communities, etc) unless Cascade is set or we clean up.
            // Our cleanup_my_data handles it, maybe we can use similar logic or rely on CASCADE.
            // Remote schema shows ON DELETE CASCADE for most relationships (e.g. user_gyms, communities).
            const { error } = await supabase
                .from('gyms')
                .delete()
                .eq('id', gymId)
            ...
            console.error("Delete Global Gym Error:", err);
            throw err;
        }
    };
