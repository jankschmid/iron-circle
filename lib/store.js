...
export function StoreProvider({ children }) {
    ...
    useEffect(() => {
        ...
        const syncAndClean = async () => {
            ...
            // 3. Global Janitor (RPC) - Be a good citizen and help clean the DB
            // Call this less frequently or just on load? 
            // We'll call it here. It's cheap.
            try {
                // TEST MODE: 1 Minute Cleanup
                await supabase.rpc('cleanup_stale_sessions', { timeout_minutes: 1 });
            } catch (err) {
    ...
    const startWorkout = async (templateId) => {
        ...
        try {
            ...
            // This ensures if they reload immediately, they see the structure
