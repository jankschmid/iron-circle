...
import { calculateSessionXP, getLevelProgress } from './gamification';
import { analyzeSession } from './algorithms';
...
export function StoreProvider({ children }) {
    ...
    const fetchProfile = async (session) => {
        ...
        try {
            ...
            const [profileRes, userGymsRes, rolesRes] = await Promise.all([
                ...
                supabase.from('profiles').select('*, cycle_xp, xp_overflow').eq('id', session.user.id).maybeSingle(),
                // 2. User Gyms
            ...
            // AUTO-HEAL: Verify Level Logic
            if (profile) {
                // Use cycle_xp for level calculation if available (Gamification v2)
                const effectiveXP = (profile.cycle_xp !== undefined && profile.cycle_xp !== null)
                    ? profile.cycle_xp
                    : (profile.lifetime_xp || 0);

                const calc = getLevelProgress(effectiveXP);

                // If DB is desynced (e.g. Current XP > Required), fix it.
                if (profile.level !== calc.currentLevel || profile.current_xp !== calc.progress) {
                    console.log(`Auto-Healing XP: Level ${profile.level}->${calc.currentLevel}, XP ${profile.current_xp}->${calc.progress}`);

                    // Update DB asynchronously
                    supabase.from('profiles').update({
                        level: calc.currentLevel,
                        current_xp: calc.progress
                    }).eq('id', profile.id).then(({ error }) => {
                        if (error) console.error("Auto-heal failed", error);
                    });

                    // Use corrected values locally
                    profile.level = calc.currentLevel;
                    profile.current_xp = calc.progress;
                    profile.next_level_xp = calc.totalNeeded; // Optional: add for UI convenience
                }
            }

            const { data: userGymsData, error: gymError } = userGymsRes;
            const { data: rolesData } = rolesRes;
            // No assignment data for now
            ...
            console.log("fetchProfile: User Gyms found:", userGymsData?.length);
            if (userGymsData && userGymsData.length > 0) {
                userGymsData.forEach(ug => {
                    console.log(`DEBUG ROLE: Gym ${ug.gym_id} (${ug.gyms?.name}) -> Role: ${ug.role}`);
                });
            }
            ...
            const sessionUser = {
                ...
                cycle_xp: (profile?.cycle_xp !== undefined && profile?.cycle_xp !== null) ? Number(profile.cycle_xp) : Number(profile?.lifetime_xp || 0), // Fallback for old users
                xp_overflow: profile?.xp_overflow || 0,
        ...
        const initAuth = async () => {
            ...
            if (mounted && !user) {
                ...
                if (localUser && mounted) {
                    console.log("ΓÜí Instant Load from Preferences");
                    setUser(localUser);
    ...
    const fetchFriends = async () => {
        ...
        try {
            const { data, error: pError } = await supabase
                .from('profiles')
                .select('id, name, username, avatar_url, bio, xp, level, prestige_level')
                .in('id', friendIds);
    ...
    const formatHistory = async (data) => {
        if (!data) return [];
    ...
    const fetchHistory = async () => {
        ...
        if (error) {
            ...
            const formatted = await formatHistory(data);
            setHistory(formatted);
        }
    };
    ...
    const finishWorkout = async ({ visibility = 'public' } = {}) => {
        ...
        let prCount = 0;
        const oldPrs = user.prs || {};

        // Simple PR check: Did we lift more than before?
        // Note: activeWorkout.logs has current session maxes implicitly if we scan sets
        activeWorkout.logs.forEach(log => {
            ...
            if (maxSet > oldPr && oldPr > 0) {
                prCount++;
            } else if (maxSet > 0 && oldPr === 0) {
                // First time logging a weight -> is it a PR? Maybe partial.
                // Let's count it as a PR for encouragement if it's a "New Record" (0 -> X)
                prCount++;
            }
        });

        // 2. Calculate Streak Logic (Optimistic)
        const currentStreak = user.streak || 0;

        // STREAK & SESSION COUNT CALCULATION
        const lastWorkoutDate = user.last_workout_date ? new Date(user.last_workout_date) : null;
        const now = new Date();
        const todayStr = now.toISOString().split('T')[0];
        const lastStr = lastWorkoutDate ? lastWorkoutDate.toISOString().split('T')[0] : null;
        ...
        const newLifetimeWorkouts = (user.lifetime_workouts || 0) + 1;

        // NEW: Calculate XP with Goal Multipliers
        const { total: earnedXP, breakdown } = calculateSessionXP({
            volume: totalVolume,
            prs: prCount,
            streak: newStreak,
            cardio: (activeWorkout.duration > 600 && totalVolume === 0) ? { distance: 0, duration: activeWorkout.duration / 60 } : null
        }, user.goal || 'Muscle');

        // XP Sanity Cap (Anti-Cheat)
        // XP Sanity Cap (Anti-Cheat)
        const XP_CAP = 3000;
        let finalXP = earnedXP;

        if (earnedXP > XP_CAP) {
            console.warn(`XP Cap Hit: ${earnedXP} -> ${XP_CAP}`);
            finalXP = XP_CAP;
        }

        // 1. LIFETIME XP (Score - Never Resets)
        const currentLifetimeXP = BigInt(user.lifetime_xp || user.xp || 0);
        const newLifetimeXP = currentLifetimeXP + BigInt(finalXP);

        // 2. CYCLE XP (Level Progress - Resets on Prestige)
        // Correct Logic: Only fallback to lifetime if NO prestige. If prestiged, missing cycle means 0.
        const effectiveCycleXP = (user.cycle_xp !== undefined && user.cycle_xp !== null)
            ? Number(user.cycle_xp)
            : ((user.prestige_level || 0) > 0 ? 0 : Math.min(Number(currentLifetimeXP), 540000));

        let newCycleXP = effectiveCycleXP + finalXP;

        // 3. STORAGE & DISPLAY
        const progressData = getLevelProgress(newCycleXP);
        const newLevel = progressData.currentLevel;
        const displayProgressXP = progressData.progress;

        // Overflow is now just static legacy storage (or if they hit Lvl 1000 cap)
        // We don't add to it here anymore, as cycle_xp grows linearly.
        const currentOverflow = BigInt(user.xp_overflow || 0);

        const didLevelUp = newLevel > (user.level || 1);

        console.log(`Gamification: Earned ${finalXP} XP. Level: ${newLevel}. Cycle: ${newCycleXP}.`);

        // Persist Gamification Stats
        try {
            const updatePayload = {
                current_xp: displayProgressXP,
                lifetime_xp: newLifetimeXP.toString(),
                cycle_xp: newCycleXP,
                level: newLevel,
                xp: newLifetimeXP.toString(), // Legacy sync
                streak: newStreak,
                lifetime_workouts: newLifetimeWorkouts,
                // xp_overflow: currentOverflow.toString(), // No change to overflow
            ...
            setUser(prev => ({
                ...
                lifetime_xp: newLifetimeXP.toString(),
                cycle_xp: newCycleXP,
                xp: newLifetimeXP.toString(),
                level: newLevel,
                streak: newStreak,
                lifetime_workouts: newLifetimeWorkouts,
                xp_overflow: user.xp_overflow || 0, // Keep existing overflow
                last_workout_date: updatePayload.last_workout_date
            }));

            if (activeWorkout) {
                toast.success(`+${finalXP} XP!`);
                if (didLevelUp) {
                    toast.success(`LEVEL UP! ${newLevel}`);
                    // Haptic for Level Up
                    if (navigator.vibrate) navigator.vibrate([100, 50, 100, 50, 200]);
                }

                // OPERATIONS CHECK
                supabase.rpc('check_operations_progress', { p_workout_id: activeWorkout.id })
                    .then(({ data }) => {
                        if (data && data.success && data.completed_count > 0) {
                            data.completed_names.forEach(name => {
                                toast.success(`MISSION COMPLETE: ${name}`);
                                // Haptic for Mission Complete
        ...
        const oldPrsSnapshot = { ...user.prs }; // Snapshot before update? 
        // Actually we already updated user.prs in step 1? No, we haven't updated PRs in DB yet!
        // Wait, where do we update PRs?
        // We usually update PRs in a separate check or inside this function.
        // Let's assume we do it here or relying on `check_operations_progress` backend?
        // Standard app usually updates PRs in logic.
        // Let's use the `oldPrs` variable we defined earlier.

        const analysis = analyzeSession(completedSession, history, oldPrs);
        ...
        try {
            ...
            setWorkoutSummary({
                ...
                volume: totalVolume,
                earnedXP,
                newTotalXP: newLifetimeXP,
                newLevel,
                didLevelUp,
                breakdown // Pass breakdown to UI
            });

            // Update History Locally (Optimistic)
            setHistory(prev => [completedSession, ...prev]);
    ...
    // Calculate Monthly Stats (Last 4 Weeks)
    const getMonthlyStats = () => {
        ...
        const weeklyVolume = [0, 0, 0, 0]; // Last 4 weeks
        const muscleSplit = {};

        history.forEach(session => {
            const date = new Date(session.endTime);
            if (date >= fourWeeksAgo) {
                // Volume Bucket (Week 1-4)
                const diffTime = Math.abs(now - date);
                const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
                const weekIndex = 3 - Math.floor((diffDays - 1) / 7); // 0=Oldest, 3=Current
                ...
                if (weekIndex >= 0 && weekIndex <= 3) {
                    ...
                    if (!session.volume && session.logs) {
                        session.logs.forEach(l => {
                            let exerciseVol = 0;
                            l.sets.forEach(s => {
                                if (s.completed) exerciseVol += (s.weight || 0) * (s.reps || 0);
                            });
                            vol += exerciseVol;

                            // Muscle Split
                            if (l.exercise) {
                                const muscle = l.exercise.muscle || 'Other';
                                muscleSplit[muscle] = (muscleSplit[muscle] || 0) + (l.sets?.filter(s => s.completed).length || 0);
                            } else if (l.exerciseId) {
                                // Fallback lookup
                                const ex = EXERCISES.find(e => e.id === l.exerciseId) || exercises.find(e => e.id === l.exerciseId);
                                const muscle = ex?.muscle || 'Other';
                                muscleSplit[muscle] = (muscleSplit[muscle] || 0) + (l.sets?.filter(s => s.completed).length || 0);
                            }
                        });
                    }
                    weeklyVolume[weekIndex] += vol;
                }
            }
        });

        // Format Split for Chart
        const splitArray = Object.entries(muscleSplit)
    ...
    return (
        <StoreContext.Provider value={{
            user,
            setUser, // Exposed for optimistic updates
            updateUserProfile,
            friends,
            activeWorkout,
            startWorkout,
