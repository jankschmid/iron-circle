...
export function getWarmupForRoutine(template, allExercises = []) {
    ...
    template.exercises.forEach(ex => {
        // Try to find full exercise details
    ...
    if (muscles.some(m => ['Chest', 'Shoulders'].includes(m))) {
        warmupRoutine.push({ name: 'Arm Circles', duration: '30s', type: 'Stretch' });
        warmupRoutine.push({ name: 'Wall Slides', duration: '12 reps', type: 'Stretch' });
    }

    // LEGS / GLUTES
    if (muscles.some(m => ['Legs', 'Glutes', 'Calves'].includes(m))) {
        warmupRoutine.push({ name: 'Leg Swings', duration: '15 reps/side', type: 'Stretch' });
        warmupRoutine.push({ name: 'Deep Squat Hold', duration: '30s', type: 'Stretch' });
    }

    // BACK
    if (muscles.includes('Back')) {
        warmupRoutine.push({ name: 'Cat-Cow', duration: '10 reps', type: 'Stretch' });
        warmupRoutine.push({ name: 'Thoracic Rotation', duration: '8 reps/side', type: 'Stretch' });
    }
