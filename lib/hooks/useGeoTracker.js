...
export function useGeoTracker() {
    ...
    // Request Permission & Start Watch when user is available
    // We always track location when logged in for the distance display
    // and geofence auto-start. The auto-start/stop logic is gated by user.auto_tracking_enabled
    // inside checkGeofence.
    useEffect(() => {
        if (!user) return; // Need a user at minimum
    ...
    }, [user?.id, user?.auto_tracking_enabled, workoutSession?.id]); // Restart if user changes (login/logout) or track state changes
    ...
    const checkGeofence = async (lat, lng) => {
        ...
        for (const gym of gyms) {
            ...
            gymsWithDistance.push({ ...gym, dist_meters: dist });
        ...
        if (nearestGym) {
            // Use custom radius if set, or default to 200m
            ...
            if (user.auto_tracking_enabled) {
                if (isInside) {
                    ...
                    // If we are outside, clear any suppression for this nearest gym
                    const suppressedGymId = sessionStorage.getItem('suppressedGymId');
                    if (suppressedGymId === nearestGym.id) {
        ...
        // STOP LOGIC (Decoupled from nearestGym)
        if (user.auto_tracking_enabled && workoutSession && workoutSession.type === 'auto') {
            const activeGym = gymsWithDistance.find(g => g.id === workoutSession.gym_id);
            if (activeGym) {
                const radius = activeGym.radius || 200;
                const isOutside = activeGym.dist_meters > (radius + 50); // Hysteresis

                if (isOutside) {
                    console.log('[GeoTracker] Auto-stopping session (left tracked gym)');
                    stopTrackingSession('auto-leave');
                }
            } else {
                // Active gym not found in nearby checks (likely too far or location missing)
                console.log('[GeoTracker] No active gym nearby, auto-stopping session');
                stopTrackingSession('auto-leave');
            }
        }
    };
