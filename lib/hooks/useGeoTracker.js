...
export function useGeoTracker() {
    ...
    const checkGeofence = async (lat, lng) => {
        ...
        console.log('[GeoTracker] Checking geofence:', { lat, lng, gymsCount: gyms.length, autoTracking: user?.auto_tracking_enabled });
        ...
        for (const gym of gyms) {
            if (!gym.location) {
                console.log('[GeoTracker] Gym has no location:', gym);
                continue; // Skip if missing location data
            }
            ...
            if (!gLat || !gLng) {
                console.log('[GeoTracker] Could not parse gym location:', gym.location);
                continue;
            }

            const dist = getDistanceFromLatLonInM(lat, lng, gLat, gLng);
            console.log('[GeoTracker] Distance to', gym.label || gym.name, ':', Math.round(dist), 'm');
        ...
        if (nearestGym) {
            ...
            console.log('[GeoTracker] Nearest gym:', nearestGym.label || nearestGym.name, 'Distance:', Math.round(nearestGym.dist_meters), 'm', 'isInside:', isInside);
            ...
            if (user.auto_tracking_enabled) {
                if (isInside) {
                    ...
                    if (workoutSession && workoutSession.gym_id === nearestGym.id) {
                        ...
                        console.log('[GeoTracker] Already tracking this gym');
                        ...
                        console.log('[GeoTracker] Already tracking another gym, ignoring');
                    }
