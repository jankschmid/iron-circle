...
export const XP_EVENTS = {
    WORKOUT_COMPLETE: 500,    // Base completion
    GYM_CHECKIN: 100,         // Passive tracker session
    PR_HIT: 500,              // Per PR in a session (handled by DB RPC now)
    REFERRAL: 1000
};

/**
 * Returns the XP multiplier tier based on current streak count.
 * Streak is frozen → returns 0 (no bonus XP)
 * 1–4   → 1.00x (No boost)
 * 5–9   → 1.10x (+10%)
 * 10–19 → 1.25x (+25%)
 * 20+   → 1.50x (+50% MAX)
 */
export function getStreakMultiplier(currentStreak, wasFrozen = false) {
    if (wasFrozen) return 0;
    if (currentStreak >= 20) return 1.50;
    if (currentStreak >= 10) return 1.25;
    if (currentStreak >= 5) return 1.10;
    return 1.00;
}

export function getStreakTierLabel(currentStreak, wasFrozen = false) {
    if (wasFrozen) return '❄️ Frozen (0x bonus)';
    if (currentStreak >= 20) return `🔥 ${currentStreak}-Day Streak (1.5x)`;
    if (currentStreak >= 10) return `🔥 ${currentStreak}-Day Streak (1.25x)`;
    if (currentStreak >= 5) return `🔥 ${currentStreak}-Day Streak (1.1x)`;
    return `${currentStreak}-Day Streak (No bonus yet)`;
}
...
export function calculateSessionXP(sessionData, userGoal = 'Muscle') {
    ...
    if (sessionData.volume > 0) {
        ...
        // Scale Volume Impact: 0.25 yields 500XP for 2000kg, 1250XP for 5000kg
        let volXP = Math.floor(cappedVolume * 0.25 * mult.volume);

        // Anti-Cheat: Cap Max Volume XP (e.g. 10000 XP)
        if (volXP > 10000) volXP = 10000;
    ...
    if (sessionData.cardio) {
        ...
        let cardioXP = 0;
        if (distance > 0) cardioXP += Math.floor(distance * 500); // 1km = 500xp (up from 100)
        if (duration > 0) cardioXP += Math.floor(duration * 10);   // 1min = 10xp (up from 2)

        // Apply Multiplier
        cardioXP = Math.floor(cardioXP * mult.cardio);

        // Anti-Cheat: Cap Cardio XP
        if (cardioXP > 7500) cardioXP = 7500;
    ...
    // 4. Streak Multiplier (applied to base+volume+cardio, not PR bonus)
    const streakMultiplier = getStreakMultiplier(
        sessionData.streak || 0,
        sessionData.wasStreakFrozen || false
    );

    if (sessionData.wasStreakFrozen) {
        // Show the freeze state explicitly as a 0 XP line
        breakdown.push({ label: getStreakTierLabel(sessionData.streak || 0, true), value: 0, isFrozen: true });
    } else if (streakMultiplier > 1.0) {
        const streakBonus = Math.floor(xp * (streakMultiplier - 1));
        xp += streakBonus;
        breakdown.push({
            label: getStreakTierLabel(sessionData.streak || 0),
            value: streakBonus
        });
    } else if ((sessionData.streak || 0) > 0) {
        // Show streak even if no bonus yet
        breakdown.push({ label: getStreakTierLabel(sessionData.streak || 0), value: 0 });
    }

    // HARD CAP for entire session (e.g. 20000 XP)
    if (xp > 20000) {
        xp = 20000;
